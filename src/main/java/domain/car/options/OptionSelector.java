 ...
 * After selecting each option, it updates the available selectedOptions on other categories
 */
public class OptionSelector {
     ...
     * @return null if the selected option is not a valid choice, otherwise the new selectedOptions after selecting this option
     */
    private Map<OptionCategory, List<Option>> getNewOptions(Option option) {
        var category = option.category();
        Map<OptionCategory, List<Option>> newOptions;
        // There might be a rule associated with this category, which filters other selectedOptions. We apply this first
    ...
    /**
     * Select this option and set it in the selected selectedOptions
     *
     * @param option The option to select for this category
    ...
    /**
     * @return The categories and their selectedOptions for which you still need to select an option
     */
    ...
    /**
     * If you are finished, you can call this method to get the selectedOptions that you selected
    ...
    public Map<OptionCategory, Option> getSelectedOptions() {
        if (options.size() != selectedOptions.size())
            throw new IllegalStateException("Can't get selected selectedOptions if not all selectedOptions are selected yet!");
        return new LinkedHashMap<>(selectedOptions);  // Return a copy of the selected selectedOptions
    }
}
